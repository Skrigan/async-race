...
class Loader {
	...
	constructor(public baseLink: string) {
		...
		(document.querySelector('.form_create') as HTMLFormElement).addEventListener('submit', (e) => this.createCar(e));
		(document.querySelector('.form_update') as HTMLFormElement).addEventListener('submit', (e) => this.updateCar(e));
	...
	// load(endpoint: string, callback: any) {
	// 	fetch(`${this.baseLink}/${endpoint}`)
	// 		.then((res) => res.json())
